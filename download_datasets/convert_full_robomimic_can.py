...
parser = argparse.ArgumentParser(description='Convert robomimic CAN dataset to DINO_WM format with FULL 39D proprio')
...
def convert_robomimic_can_full(
    source_dir="/home/ubuntu/minghao/data/robomimic/can/ph",
    target_dir="/home/ubuntu/minghao/data/robomimic/can/ph_convert_full"
):
    """
    Robomimic CAN任务到DINO_WM格式转换 - 使用完整的43D proprio信息

    完整维度设计：
    - Actions: 7D (完整的机械臂 + 夹爪控制)
    - Proprio: 43D (使用所有robot0变量)
    - States: 57D (43D proprio + 14D object)
    ...
    总计: 43D proprio + 14D object = 57D完整状态
    """

    print("=== ROBOMIMIC CAN TO DINO_WM (FULL 43D PROPRIO) CONVERSION ===")
    ...
    state_dim = proprio_dim + object_dim  # 57D完整状态
    ...
    for i, demo_key in enumerate(tqdm(demo_keys)):
        ...
        # 构建完整State (57D): Proprio + 完整物体状态
        state = np.concatenate([
            proprio,        # 43D: 完整机器人状态
            object_state    # 14D: 完整物体状态
        ], axis=1)
    ...
    # 验证数据格式
    print("\n=== FULL 39D PROPRIO DATA FORMAT VERIFICATION ===")
    ...
    print(f"\n=== COMPLETE STATE COMPONENTS (57D) ===")
    print(f"Proprio: {proprio_dim}D (完整机器人状态)")
    print(f"Object: {object_dim}D (完整物体状态)")
    print(f"总计: {state_dim}D完整状态")

    # 与旧版本比较
    print(f"\n=== 与旧版本对比 ===")
    print(f"旧版本: Actions 7D, Proprio 16D, States 23D")
...
if __name__ == "__main__":
    convert_robomimic_can_full(args.source_dir, args.save_data_dir)