...
parser = argparse.ArgumentParser(description='Convert robomimic LIFT dataset to DINO_WM format with FULL 39D proprio')
...
def convert_robomimic_lift_full(
    source_dir="/home/ubuntu/minghao/data/robomimic/lift/ph",
    target_dir="/home/ubuntu/minghao/data/robomimic/lift/ph_convert_full"
):
    """
    Robomimic LIFT任务到DINO_WM格式转换 - 使用完整的43D proprio信息

    完整维度设计：
    - Actions: 7D (完整的机械臂 + 夹爪控制)
    - Proprio: 43D (使用所有robot0变量)
    - States: 53D (43D proprio + 10D object)
    ...
    - object (10D): 完整物体状态 (LIFT特有)

    总计: 43D proprio + 10D object = 53D完整状态
    """

    print("=== ROBOMIMIC LIFT TO DINO_WM (FULL 43D PROPRIO) CONVERSION ===")
    ...
    print(f"object: {object_dim}D (LIFT任务特有)")
    ...
    state_dim = proprio_dim + object_dim  # 53D完整状态
    ...
    for i, demo_key in enumerate(tqdm(demo_keys)):
        ...
        # 提取完整物体状态 (LIFT任务特有的10D)
        object_state = low_dim_obs['object'][:]  # (seq_len, 10)

        # 构建完整State (53D): Proprio + 完整物体状态
        state = np.concatenate([
            proprio,        # 43D: 完整机器人状态
            object_state    # 10D: LIFT任务完整物体状态
        ], axis=1)
    ...
    # 验证数据格式
    print("\n=== FULL 39D PROPRIO DATA FORMAT VERIFICATION (LIFT) ===")
    ...
    print(f"\n=== COMPLETE STATE COMPONENTS (53D) ===")
    print(f"Proprio: {proprio_dim}D (完整机器人状态)")
    print(f"Object: {object_dim}D (LIFT任务完整物体状态)")
    print(f"总计: {state_dim}D完整状态")

    # 与旧版本比较
    print(f"\n=== 与旧版本对比 ===")
    print(f"旧版本: Actions 7D, Proprio 16D, States 26D")
    print(f"新版本: Actions {action_dim}D, Proprio {proprio_dim}D, States {state_dim}D")
    print(f"改进: 信息利用率从 41% 提升到 100%")
    print(f"✅ 包含所有末端执行器信息")
    print(f"✅ 包含所有关节编码信息")
    print(f"✅ 包含所有动态信息")
    print(f"✅ 完整的LIFT物体状态表示")

    # 与CAN任务对比
    print(f"\n=== LIFT vs CAN 任务对比 ===")
    print(f"CAN: 43D proprio + 14D object = 57D states")
    print(f"LIFT: 43D proprio + 10D object = 53D states")
    print(f"区别: LIFT物体状态比CAN少4维 (task-specific)")

    return target_path

if __name__ == "__main__":
    convert_robomimic_lift_full(args.source_dir, args.save_data_dir)