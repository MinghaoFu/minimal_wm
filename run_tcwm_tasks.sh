...
PID_FILE="logs/train_tcwm.pids"
JOB_FILE="logs/train_tcwm.jobs"
...
for task in "${TASKS[@]}"; do
    ...
    if [ -n "$gpu" ]; then
        nohup env CUDA_VISIBLE_DEVICES="$gpu" DATASET_NAME="$task" ./train_tcwm.sh > "$log_file" 2>&1 &
    else
        nohup env DATASET_NAME="$task" ./train_tcwm.sh > "$log_file" 2>&1 &
    fi
    pid=$!
    echo "$pid" >> "$PID_FILE"
