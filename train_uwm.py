...
class Trainer:
    ...
    def init_models(self):
        ...
        if emb_decoder_target == "models.emb_decoder_identity.EmbDecoderIdentity":
        ...
        if hasattr(self.cfg, "alignment") and self.cfg.alignment.open_alignment:
        ...
        self.model = hydra.utils.instantiate(
            ...
            open_alignment=self.cfg.alignment.open_alignment if hasattr(self.cfg, "alignment") else False,
        )
        ...
        if hasattr(self.cfg, "alignment") and self.cfg.alignment.open_alignment and self.alignment_projection is not None:
            self.model.open_alignment = True
        else:
            self.model.open_alignment = False
    ...
    def openloop_rollout(
        ...
        if rollout_parallel > 1 and traj_indices is not None:
            try:
                with ProcessPoolExecutor(
                    ...
                    prepared_samples = [f.result() for f in futures]
            except Exception as exc:
                log.warning("Parallel rollout sampling failed, falling back to sequential: %s", exc)
                prepared_samples = None
        ...
        if self.accelerator.is_main_process:
            if (
                ...
                grid_row_gap = getattr(self.cfg, "video_grid_row_gap", 0)
                ...
                for postfix, entries in grid_sequences.items():
                    ...
                    entries = entries[: num_rollout]
                    if grid_cols is None:
                        grid_cols = len(entries)
                    ...
                    for t in range(max_t):
                        ...
                        for chunk_start in range(0, len(entries), num_cols):
                            ...
                            for e in chunk:
                                ...
                                gt_row.append(gt_seq[idx_t])
                                pred_row.append(pred_seq[idx_p])
                            if len(chunk) < num_cols:
                                ...
                                for _ in range(pad_count):
                                    gt_row.append(pad)
                                    pred_row.append(pad)
                            gt_row = torch.cat(gt_row, dim=2)
                            pred_row = torch.cat(pred_row, dim=2)
                            rows.append(gt_row)
                            rows.append(pred_row)
                            if grid_row_gap > 0 and (chunk_start + num_cols) < len(entries):
                                gap_row = torch.zeros(
                                    (gt_row.shape[0], grid_row_gap, gt_row.shape[2]),
                                    dtype=gt_row.dtype,
                                    device=gt_row.device,
                                )
                                rows.append(gap_row)
                        grid = torch.cat(rows, dim=1)
                        if self.video_resize is not None:
                            base_h = gt_row.shape[1]
                            gap_scaled = 0
                            if base_h > 0 and grid_row_gap > 0:
                                gap_scaled = int(round(self.video_resize * grid_row_gap / base_h))
                                if gap_scaled < 0:
                                    gap_scaled = 0
                            grid = torch.nn.functional.interpolate(
                                grid.unsqueeze(0),
                                size=(
                                    self.video_resize * 2 * num_chunks + gap_scaled * max(0, num_chunks - 1),
                                    self.video_resize * num_cols,
                                ),
                                mode="bilinear",
                                align_corners=False,
                            ).squeeze(0)
