...
parser = argparse.ArgumentParser(description='Convert robomimic TRANSPORT dataset to DINO_WM format with FULL 43D proprio')
...
def convert_robomimic_transport_full(
    source_dir="/data2/minghao/data/robomimic/transport/ph",
    target_dir="/data2/minghao/data/robomimic/transport/ph_convert_full"
):
    """
    Robomimic TRANSPORT任务到DINO_WM格式转换 - 使用完整的43D proprio信息

    完整维度设计：
    - Actions: 7D (完整的机械臂 + 夹爪控制)
    - Proprio: 43D (使用所有robot0变量)
    - States: 43D + object_dim (43D proprio + object)
    ...
    - robot0_eef_quat_site (4D): 备用末端方向
    - robot0_gripper_qvel (2D): 夹爪速度
    - robot0_joint_pos_cos (7D): 关节角度余弦
    - robot0_joint_pos_sin (7D): 关节角度正弦
    - object: 完整物体状态 (TRANSPORT特有)

    总计: 43D proprio + object_dim = 完整状态
    """

    print("=== ROBOMIMIC TRANSPORT TO DINO_WM (FULL 43D PROPRIO) CONVERSION ===")
    ...
    # 检查并打印所有robot变量的维度 (TRANSPORT有两个机器人)
    print("\n=== 完整PROPRIO变量分析 (双臂任务) ===")
    robot_vars = {}
    total_proprio_dim = 0

    for key in sorted(sample_obs.keys()):
        if 'robot0' in key or 'robot1' in key:
            data = sample_obs[key][:]
            robot_vars[key] = data.shape[1]
    ...
    print(f"object: {object_dim}D (TRANSPORT任务特有)")

    print(f"\n总PROPRIO维度 (robot0 + robot1): {total_proprio_dim}D")
    print(f"物体状态维度: {object_dim}D")
    print(f"完整状态维度: {total_proprio_dim + object_dim}D")

    # 维度设计 (TRANSPORT是双臂任务)
    action_dim = sample_actions.shape[1]  # 获取实际动作维度 (14D for two-arm)
    proprio_dim = total_proprio_dim  # 完整proprio (包含两个机器人)
    state_dim = proprio_dim + object_dim  # 完整状态
    vel_dim = 7  # 单个机器人的关节速度
    ...
    print(f"State dimension: {state_dim} (proprio + transport object state)")
    ...
    for i, demo_key in enumerate(tqdm(demo_keys)):
        ...
        # 构建完整Proprio: 包含所有robot0和robot1变量 (双臂任务)
        proprio_components = []

        # 按照固定顺序添加所有robot变量 (robot0 和 robot1)
        all_robot_keys = [
            'robot0_joint_pos', 'robot0_gripper_qpos', 'robot0_joint_vel',
            'robot0_eef_pos', 'robot0_eef_quat', 'robot0_eef_quat_site',
            'robot0_gripper_qvel', 'robot0_joint_pos_cos', 'robot0_joint_pos_sin',
            'robot1_joint_pos', 'robot1_gripper_qpos', 'robot1_joint_vel',
            'robot1_eef_pos', 'robot1_eef_quat', 'robot1_eef_quat_site',
            'robot1_gripper_qvel', 'robot1_joint_pos_cos', 'robot1_joint_pos_sin'
        ]
    ...
    # 验证数据格式
    print("\n=== FULL 43D PROPRIO DATA FORMAT VERIFICATION (TRANSPORT) ===")
    ...
    # 显示详细组成
    print(f"\n=== COMPLETE PROPRIO COMPONENTS (双臂任务: {proprio_dim}D) ===")
    print(f"Robot0 (43D):")
    ...
    print(f"  - robot0_eef_quat_site (4D): 备用末端方向")
    print(f"  - robot0_gripper_qvel (2D): 夹爪速度")
    print(f"  - robot0_joint_pos_cos (7D): 关节角度余弦")
    print(f"  - robot0_joint_pos_sin (7D): 关节角度正弦")
    print(f"Robot1 (43D):")
    print(f"  - robot1_joint_pos (7D): 关节角度")
    print(f"  - robot1_gripper_qpos (2D): 夹爪位置")
    print(f"  - robot1_joint_vel (7D): 关节速度")
    print(f"  - robot1_eef_pos (3D): 末端执行器位置")
    print(f"  - robot1_eef_quat (4D): 末端执行器方向")
    print(f"  - robot1_eef_quat_site (4D): 备用末端方向")
    print(f"  - robot1_gripper_qvel (2D): 夹爪速度")
    print(f"  - robot1_joint_pos_cos (7D): 关节角度余弦")
    print(f"  - robot1_joint_pos_sin (7D): 关节角度正弦")
    print(f"总计: {proprio_dim}D完整proprio (86D = 43D×2 robots)")

    print(f"\n=== COMPLETE STATE COMPONENTS ({state_dim}D) ===")
    print(f"Proprio: {proprio_dim}D (完整机器人状态)")
    print(f"Object: {object_dim}D (TRANSPORT任务完整物体状态)")
    print(f"总计: {state_dim}D完整状态")

    # 与旧版本比较
    print(f"\n=== 与旧版本对比 ===")
    print(f"新版本: Actions {action_dim}D, Proprio {proprio_dim}D, States {state_dim}D")
    print(f"改进: 信息利用率提升到 100%")
    print(f"✅ 包含所有末端执行器信息")
    print(f"✅ 包含所有关节编码信息")
    print(f"✅ 包含所有动态信息")
    print(f"✅ 完整的TRANSPORT物体状态表示")

    # 与其他任务对比
    print(f"\n=== TRANSPORT vs 其他任务对比 ===")
    print(f"CAN: 43D proprio + 14D object = 57D states")
    print(f"LIFT: 43D proprio + 10D object = 53D states")
    print(f"SQUARE: 43D proprio + 14D object = 57D states")
    print(f"TRANSPORT: 43D proprio + {object_dim}D object = {state_dim}D states")

    return target_path

if __name__ == "__main__":
    convert_robomimic_transport_full(args.source_dir, args.save_data_dir)