...
parser = argparse.ArgumentParser(description='Convert robomimic TOOL_HANG dataset to DINO_WM format with FULL 43D proprio')
...
def convert_robomimic_tool_hang_full(
    source_dir="/data2/minghao/data/robomimic/tool_hang/ph",
    target_dir="/data2/minghao/data/robomimic/tool_hang/ph_convert_full"
):
    """
    Robomimic TOOL_HANG任务到DINO_WM格式转换 - 使用完整的43D proprio信息

    完整维度设计：
    - Actions: 7D (完整的机械臂 + 夹爪控制)
    - Proprio: 43D (使用所有robot0变量)
    - States: 87D (43D proprio + 44D object)
    ...
    总计: 43D proprio + 44D object = 87D完整状态
    """

    print("=== ROBOMIMIC TOOL_HANG TO DINO_WM (FULL 43D PROPRIO) CONVERSION ===")
    ...
    state_dim = proprio_dim + object_dim  # 87D完整状态
    ...
    for i, demo_key in enumerate(tqdm(demo_keys)):
        ...
        # 提取完整物体状态 (44D for tool_hang)
        object_state = low_dim_obs['object'][:]  # (seq_len, 44)

        # 构建完整State (87D): Proprio + 完整物体状态
        state = np.concatenate([
            proprio,        # 43D: 完整机器人状态
            object_state    # 44D: 完整物体状态
        ], axis=1)
    ...
    # 验证数据格式
    print("\n=== FULL 43D PROPRIO DATA FORMAT VERIFICATION ===")
    ...
    print(f"\n=== COMPLETE STATE COMPONENTS (87D) ===")
    print(f"Proprio: {proprio_dim}D (完整机器人状态)")
    print(f"Object: {object_dim}D (TOOL_HANG任务完整物体状态)")
    ...
    print(f"TRANSPORT: 86D proprio + 41D object = 127D states (双臂)")
    print(f"TOOL_HANG: 43D proprio + 44D object = 87D states")

    # Final verification
    print(f"\n=== Final verification ===")
    cap = cv2.VideoCapture(str(target_path / "obses" / "episode_00000.mp4"))
    ret, frame = cap.read()
    if ret and frame.mean() > 10:
        print(f'✅ Videos have content (mean pixel: {frame.mean():.2f})')
    else:
        print(f'❌ Videos are black!')
    cap.release()

    return target_path

if __name__ == "__main__":
    convert_robomimic_tool_hang_full(args.source_dir, args.save_data_dir)
